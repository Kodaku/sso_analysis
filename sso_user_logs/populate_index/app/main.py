...
if __name__ == "__main__":
    ...
    user_logs_df = build_dataframe(cur)
    ...
    df_types = {}
    columns = user_logs_df.columns
    dtypes = user_logs_df.dtypes
    for column, column_type in zip(columns, dtypes):
        df_types[column] = column_type

    actions = []
    index = 1
    for i, command_row in user_logs_df.iterrows():
        command = {}
        for column in user_logs_df.columns:
            command[column] = command_row[column]
        action = {"index": {"_index": index_name, "_id": command_row["id"]}, "_op_type": "upsert"}
        doc = command
